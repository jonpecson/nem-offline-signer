...
export class UserProvider {
  ...
  constructor(public storage: Storage, public events: Events, private http: Http) {
    this.load();
    AppGlobal.url = AppGlobal.url+'/api';
    // console.log(AppGlobal.url);
  }
  ...
  login(data: any){
    let seq = this.http.get(AppGlobal.url+'/user/generate_auth_cookie/?insecure=cool&username='+data.user+'&password='+data.pass);
  ...
  reset(data: any){
    let seq = this.http.get(AppGlobal.url+'/user/retrieve_password/?insecure=cool&user_login='+data.email);
    seq.map((res:Response) => {});
  ...
  signup(data: any){
    let seq = this.http.get(AppGlobal.url+'/user/register/?insecure=cool&nonce='+data.nonce+'&first_name='+data.name+'&display_name='+data.name+'&username='+data.user+'&user_pass='+data.pass+'&email='+data.email);
  ...
  update(data: any){
    ...
    let seq = this.http.get(AppGlobal.url+'/user/update_user_meta_vars/?insecure=cool&cookie='+cookie+f+l+b);
    seq.map((res:Response) => res.json());
  ...
  nonce(controller, method){
    let seq = this.http.get(AppGlobal.url+'/get_nonce/?controller='+controller+'&method='+method);
    seq.map((res:Response) => res.json());
  ...
  isLoggedIn(){
    let cookie = this.user.cookie;
    let seq = this.http.get(AppGlobal.url+'/user/validate_auth_cookie/?insecure=cool&cookie='+cookie);
    seq.map((res:Response) => res.json());
    return seq;
  }
